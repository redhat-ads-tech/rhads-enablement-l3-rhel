...
cd $TMPDIR && \
sudo csplit -z -f cert- -b %02d.pem /etc/letsencrypt/live/rhtas.{base_hostname}/fullchain.pem '/.*BEGIN CERTIFICATE.*/' '{*}' && \
sudo cat cert-00.pem cert-01.pem > /home/lab-user/server.pem && \
sudo cat cert-02.pem > /home/lab-user/root.pem && \
sudo cp /etc/letsencrypt/live/rhtas.{base_hostname}/privkey.pem /home/lab-user && \
