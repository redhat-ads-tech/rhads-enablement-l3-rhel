...
+
This uses the API endpoints exposed by RHTPA to ingest the SBOM which you can use in your own CI pipelines (You can also do the same from the web interface). There are many more APIs exposed by RHTPA which you can use to manage your software supply chain. You can see the various API endpoints by clicking on the _API_ tab (which takes you to an interactive https://rhtpa.{base_hostname}/openapi[OpenAPI specification for RHTPA endpoints^]):
