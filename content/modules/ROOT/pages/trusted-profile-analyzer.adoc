...
ansible-navigator \# <1>
  ...
  --pae=false \# <7>
  -i inventory \# <8>
  -e tpa_single_node_registry_username="$REGISTRY_USERNAME" \# <9>
  -e tpa_single_node_registry_password="$REGISTRY_PASSWORD" # <10>
----
...
<9> The registry username to use
<10> The registry password to use
...
=== Inspect Trusted Profile Analyzer installation
...
----
+
This will run the `podman pod ps` command on the `rhtpa` node. You should see:
+
[source,console]
----
...
. Retrieve logs of the services (e.g. the server service) by running the following command:
+
[source,bash,role="execute"]
----
...
+
RHTPA components are deployed using https://docs.podman.io/en/latest/markdown/podman-kube-play.1.html[`podman kube play`^]. The playbook creates a Kubernetes-style deployment for each of the services in the `/etc/rhtpa/manifests` directory on the target machine, along with ConfigMaps and Secrets in `/etc/rhtpa/configs` directory, and then runs them as Podman containers on the target machine via _systemd_ services (defined in the `/etc/systemd/system` directory on the target machine).
+
[NOTE]
====
Although these are Kubernetes objects, Podman is not a complete Kubernetes implementation, e.g. there is no control plane, Kubernetes API, or scheduler. `podman kube play` offers a bridge between Podman and Kubernetes, simplifying the workflow between the two.
====

. Take a look at the `server-pod` systemd service's _ConfigMap_, _Secret_, and _Deployment_ Kubernetes resources by running the following commands:
+
[source,bash,role="execute"]
----
echo "--------------------------------"
echo "TPA server Deployment:"
echo "--------------------------------"
ssh rhtpa "sudo cat /etc/rhtpa/manifests/Deployments/trustification_server.yaml"
echo "--------------------------------"
echo "TPA server ConfigMap:"
echo "--------------------------------"
ssh rhtpa "sudo cat /etc/rhtpa/manifests/ConfigMaps/server-api.yaml"
echo "--------------------------------"
echo "TPA server OIDC Secret:"
echo "--------------------------------"
ssh rhtpa "sudo cat /etc/rhtpa/manifests/Secrets/oidc.yaml"
echo "--------------------------------"
----
+
If you are familiar with Kubernetes, these files should look very familiar. `podman kube play` creates pods and containers, creates volumes (e.g. for ConfigMaps), and sets up networking policies between services.
+
You will find similar files for the other services in `/etc/rhtpa`.
+
[NOTE]
====
It is recommended to manage the configuration of RHTPA through the supported Ansible playbooks and roles. If you modify the configuration files manually, they will likely be overwritten during the next installation or upgrade. Instead, alter the playbook and re-run it to apply the changes.
====

. You can inspect the individual RHTPA services using traditional _systemd_ commands. For example, to see the Importer service's current status, run the following command:
+
[source,bash,role="execute"]
----
ssh rhtpa "sudo systemctl status importer.service" | head -9
----
+
This will show the current status of the Importer service. Other services can be managed in a similar manner (e.g. `server.service`, etc). For example, to restart the Importer service, run the following command:
+
[source,bash,role="execute"]
----
ssh rhtpa "sudo systemctl restart importer.service"
----
+
This will restart the Importer service.
...
To create SBOMs, we'll use https://github.com/anchore/syft[Syft^] (an open source tool for creating SBOMs). Red Hat provides a pre-built container for use on RHEL for syft, which we'll use to create our first SBOM.
...
----
podman run --rm \
  -e SYFT_REGISTRY_INSECURE_SKIP_TLS_VERIFY=true \# <1>
  registry.redhat.io/rh-syft-tech-preview/syft-rhel9:1.29.0 \# <2>
  registry:ttl.sh/rhtas/test-image-$GUID:3h \# <3>
  -o spdx-json@2.3 | tee /tmp/test-sbom.json | jq # <4>
----
<1> Since we are using locally-signed certs, skip TLS verification when pulling the test image from the ttl.sh registry.
<2> The pre-built `syft` container from Red Hat.
<3> The test image to create an SBOM for.
<4> The output is written to a file with `tee` and then piped to `jq` to pretty print the JSON. 

=== Ingest the SBOM into RHTPA

RHTPA exposes an API that you can use to ingest SBOMs. You could use raw `curl` commands to do the ingestion, but we'll use an Open ID Connect tool called `oidc-cli` to handle the authentication.

. Install the `oidc-cli` tool with these commands:
+
[source,bash,role="execute"]
----
...
----
+
. Using `oidc-cli`, create a short-lived confidential client session in Keycloak for you to use to authenticate to the RHTPA API:
+
[source,bash,role="execute"]
----
...
----
+
With this, we can conveniently generate the necessary authorization token using `odc-cli token` command, which we'll do next.

. Ingest the SBOM from `/tmp/test-sbom.json` into RHTPA:
+
[source,bash,role="execute"]
----
curl -s \# <1>
  -H "Authorization:$(oidc-cli token trusty -b)" \# <2>
  -d @/tmp/test-sbom.json \# <3>
  https://rhtpa.${BASE_HOSTNAME}/api/v2/sbom | jq # <4>
----
<1> Don't show progress output
<2> Generate the authorization token to use to authenticate to the RHTPA API and put it in the `Authorization` header.
<3> The SBOM file to ingest.
<4> The API endpoint to use to ingest the SBOM. The output is pretty printed using `jq`.
+
...
----
{
  ...
  "document_id": "https://anchore.com/syft/image/ttl.sh/[...]"
}
----
+
The output contains a unique ID for the entry and the ingested SBOM.

. Head back to the https://rhtpa.{base_hostname}[RHTPA web interface^], reload the page and you should how have 1 SBOM in the database:
+
...
+
You should see the SBOM details, including the packages and their vulnerabilities. This is a super simple SBOM and since there are no packages (other than itself) and no dependencies, we should expect no vulnerabilities.

=== Ingest a More Interesting SBOM

Let's try again with a more interesting SBOM. We'll use the `registry.access.redhat.com/ubi10/httpd-24` image, which is a Red Hat Enterprise Linux 10 image with the HTTPD package installed.
...
podman run --rm \
  -e SYFT_REGISTRY_INSECURE_SKIP_TLS_VERIFY=true \# <1>
  registry.redhat.io/rh-syft-tech-preview/syft-rhel9:1.29.0 \# <2>
  registry:registry.access.redhat.com/ubi10/httpd-24 \# <3>
  -o spdx-json@2.3 | \# <4>
  curl -s -H "Authorization:$(oidc-cli token trusty -b)" -d @- \# <5>
  "https://rhtpa.${BASE_HOSTNAME}/api/v2/sbom" | jq # <6>
----
<1> Since we are using locally-signed certs, skip TLS verification when pulling the test image from the Red Hat registry.
<2> The pre-built `syft` container from Red Hat.
<3> The more interesting image to create an SBOM for.
<4> The output format for the SBOM. The output is then piped to `curl` to ingest the SBOM into RHTPA using the `/api/v2/sbom` endpoint.
<5> Add the authorization token to the request to authenticate to the RHTPA API (generated with the `oidc-cli` command as before)
<6> The output is pretty printed using `jq`.
+
You will see a giant JSON object of the resulting SBOM (and there will be lots of `Invalid reference` messages as none of of the packages are known to our mostly empty SBOM database). But back on the https://rhtpa.{base_hostname}[RHTPA web interface^], click on _SBOMs_ and you should see the new SBOM in the database:
+
...
== Enable More Importers

As stated earlier, by default, the Red Hat CSAF, Red Hat SBOM, and Quay data importers are disabled. Using the _Importers_ tab, you can enable any of these importers to populate RHTPA with more data. You can also programmatically enable or disable importers using the API endpoints exposed by RHTPA. Let's try that!

. First, retrieve the list of all importers and their current state with the following command:
+
[source,bash,role="execute"]
----
curl -s \# <1>
  -H "Authorization:$(oidc-cli token trusty -b)" \# <2>
  https://rhtpa.${BASE_HOSTNAME}/api/v2/importer | jq # <3>
----
<1> Don't show progress output
<2> The authorization token to use to authenticate to the RHTPA API.
<3> The API endpoint to use to retrieve the list of importers. The output is pretty printed using `jq`.
+
You will see a series of JSON objects representing the importers and their current state:
+
[source,json]
----
[
  {
    "name": "cve", // <4>
    "heartbeat": 1761060494629838144, // <5>
    "configuration": { // <6>
      "cve": {
        "disabled": false,
        "period": "1day",
    ...
    "state": "running", // <7>
    "lastChange": "2025-10-21T14:31:54.62919Z",
    "progress": { // <8>
      "current": 130920,
      "total": 314803,
      "percent": 0.41587913,
      "rate": 38.68673,
      "estimatedSecondsRemaining": 4753,
      "estimatedCompletion": "2025-10-21T16:47:31.73550335Z"
    }
  },
  ...
]
----
<4> The name of the importer.
<5> The last time the importer was updated.
<6> The configuration of the importer.
<7> The current state of the importer.
<8> The progress of the importer.

. Now, let's enable the Red Hat SBOM importer with the following command:
+
[source,bash,role="execute"]
----
curl -H "Authorization:$(oidc-cli token trusty -b)" \# <1>
     -H "Content-Type: application/json" \# <2>
     -w '\nResult: %{http_code}\n' \# <3>
     -X PUT \# <4>
     -d 'true' \# <5>
    https://rhtpa.${BASE_HOSTNAME}/api/v2/importer/redhat-sboms/enabled # <6>
----
<1> The authorization token to use to authenticate to the RHTPA API.
<2> The content type to use for the request.
<3> The output is written to the terminal with the HTTP status code.
<4> HTTP PUT method is used along with the `/enabled` suffix on the importer endpoint to enable the importer.
<5> The body of the request is the boolean value `true` to enable the importer.
<6> The RHTPA API endpoint to use to enable the `redhat-sboms` importer.
+
You should get `Result: 2xx` indicating the the importer was enabled successfully (where `2xx` is any HTTP status code in the 200-299 range). Any other status code indicates an error, with a corresponding reason, so you should carefully check that you are executing the command above and try again.

. Navigate back to the https://rhtpa.{base_hostname}[RHTPA web interface^], and click on the _Importers_ tab, you should see the Red Hat SBOM importer enabled and running:
+
image::rhtpa-importer-enabled.png[]
+
This can be used to programmatically alter the RHTPA configuration as part of an automated deployment process. After a little while you'll see more interesting results in the dashboard:
+
...
[NOTE]
====
Importers can import a large amount of data! Computing resources, and setting limitations on those resources is important to ensure the application runs stable and performs as expected. In this lab, the default resource request is 1 CPU and 8 GB of RAM, for both the importer and API server deployments. There are no resource limits by default, so if you import enough data you may hit such limits and experience errors or timeouts. Consult the https://console.redhat.com/ansible/automation-hub/repo/published/redhat/trusted_profile_analyzer/content/role/tpa_single_node/[RHTPA Ansible collection documentation^] for more information on how to configure storage and compute resources for RHTPA. 
====

== Search for Vulnerabilities

You can also search for vulnerabilities, sboms, or any other object by using the search feature.

. On the the https://rhtpa.{base_hostname}[RHTPA web interface^], click on _Search_ at the left, and try typing in `httpd` to see the various packages, sboms, and any vulnerabilities that may exist:
+
image::rhtpa-search.png[]
+
By populating RHTPA with your own SBOMs as part of your CI pipelines _for each and every build_, and enabling the public well-known importers configured in RHTPA, you can begin to evaluate the risk profile of your software supply chain on a regular basis. 
