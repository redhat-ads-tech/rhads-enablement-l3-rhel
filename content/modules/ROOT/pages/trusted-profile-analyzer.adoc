...
----
+
The last `oidc-cli` command creates a short-livedconfidential client session in Keycloak for you to use to authenticate to the RHTPA API.
...
----
+
The output contains a unique ID for the entry and the ingested SBOM. Head back to the https://rhtpa.{base_hostname}[RHTPA web interface^], reload the pageand you should how have 1 SBOM in the database:
+
